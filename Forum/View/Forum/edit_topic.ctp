...
<script type="text/javascript">
    $(function() {
        $(document).ready(function(){
            if(($.trim($("#Tags").text()).length == 0)){
                $("#array_Tags").css("display", "none");
            }
        });
    });

    function ReplaceDrawn(){
        forumtitle = $("input[name='title']").val().replace(/-/g, "_._");
...
<div class="background-forum">
    <div class="<?= $theme; ?> marge">
        <div class="row">
            <div class="col-md-12">
                <form method="post" action="<?= $this->Html->url('/'.$this->request->url); ?>" onsubmit="ReplaceDrawn()">
                    <input name="data[_Token][key]" value="<?= $csrfToken ?>" type="hidden" />
                    <?php if($params['isEdit']['title']): ?>
                        <div class="form-group">
                            <input name="title" class="form-control" value="<?= str_replace('_._', '-', $params['title']); ?>" placeholder="<?= $Lang->get('FORUM__TITLE__TOPIC'); ?>" />
                        </div>
                        <div class="forum-forum" id="array_Tags">
